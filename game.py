...
class Game(object):
    ...
    defaults = {
        ...
        "delay": 0, #seconds to sleep after each (ncurses) display update
    ...
    def __init__(self, **options):
        ...
        self.turn = 0
        self.turn_order = []
        self.turns_last_10 = []  # Track turns for every 10 games
        self.turn_counts = []  # Stores the number of turns for each game
        self.win_counts = collections.defaultdict(int)  # Tracks wins for each AI
    ...
    def play(self):
        """
        Main game loop, modified to track turns and record the winner.
        """
        ...
        for i, name in enumerate(self.players):
            ...
            player.color = COLOR_MAP.get(name, (i + 1) % 7 or 1)
            player.ord = ord(ORDERS[i % len(ORDERS)])
            player.ai.start()
        ...
        self.turn = 0  # Track the number of turns
        ...
        self.stats.record_game(winner.name, self.turn)
        self.win_counts[winner.name] += 1  # Update win count for the winner
        self.turn_counts.append(self.turn)  # Record the total number of turns for this game
        self.turns_last_10.append(self.turn)  # Store for batch tracking
        self.event(("victory", winner), player=[self.player.name])
        if len(self.turns_last_10) == 10:
                    avg_turns = sum(self.turns_last_10) / 10  # Compute average
                    with open("average_turns.txt", "a") as f:  # Append mode
                        f.write(f"After {len(self.turn_counts)} games: Average Turns = {avg_turns:.2f}\n")
                    print(f"Logged average turns after {len(self.turn_counts)} games.")

                    self.turns_last_10 = []  # Reset batch tracker
        # Notify all AIs that the game has ended
        for p in self.players.values():
            p.ai.end()
